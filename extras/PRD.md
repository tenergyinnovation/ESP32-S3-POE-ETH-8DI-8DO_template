...
## ESP32-S3-POE-ETH-8DI-8DO LoRa Gateway Project
## เอกสารข้อกำหนดผลิตภัณฑ์ (PRD) โปรเจ็ค LoRa Gateway

**Project Name**: LoRa Green Energy Data Gateway  
**ชื่อโปรเจ็ค**: ระบบเก็บรวบรวมข้อมูลพลังงานสีเขียวผ่าน LoRa  
**Version**: 1.3  
**Date**: 2026-06-26  
...
This document describes a comprehensive IoT gateway system built on the ESP32-S3-POE-ETH-8DI-8DO board. The system acts as a central hub for collecting energy data from remote LoRa transmitters, processing the data, and distributing it to multiple cloud services for storage, analysis, and real-time notifications.

**คำอธิบายภาษาไทย**: เอกสารนี้อธิบายระบบเก็บรวบรวมข้อมูล IoT ที่ครบถ้วนซึ่งสร้างขึ้นจากบอร์ด ESP32-S3-POE-ETH-8DI-8DO ระบบนี้ทำหน้าที่เป็นศูนย์กลางสำหรับเก็บรวบรวมข้อมูลพลังงานจากตัวส่งข้อมูล LoRa ที่อยู่ไกลออกไป ประมวลผลข้อมูล และกระจายข้อมูลไปยังบริการคลาวด์หลายแห่งเพื่อการเก็บข้อมูล วิเคราะห์ผล และแจ้งเตือนแบบเรียลไทม์

---

## 🎯 Project Objectives / วัตถุประสงค์ของโปรเจ็ค

1. **Data Collection**: Receive energy data from remote LoRa transmitters via SX1262 LoRa DTU
   - **ภาษาไทย**: เก็บรวบรวมข้อมูลพลังงานจากตัวส่งข้อมูล LoRa ที่อยู่ไกลออกไปผ่าน SX1262 LoRa DTU

2. **Cloud Integration**: Send data to MQTT broker and Google Sheets for centralized storage
   - **ภาษาไทย**: ส่งข้อมูลไปยัง MQTT Broker และ Google Sheets เพื่อการเก็บข้อมูลแบบรวมศูนย์

3. **User Notification**: Alert users via Telegram Bot when new data arrives
   - **ภาษาไทย**: แจ้งเตือนผู้ใช้ผ่าน Telegram Bot เมื่อมีข้อมูลใหม่เข้ามา

4. **Web Configuration**: Provide web-based WiFi configuration interface
   - **ภาษาไทย**: ให้ผู้ใช้สามารถตั้งค่า WiFi ผ่านหน้าเว็บ

5. **Reliability**: Ensure robust operation with proper error handling and failover mechanisms
   - **ภาษาไทย**: ให้ระบบทำงานได้เสถียรด้วยการจัดการข้อผิดพลาดและการสำรองข้อมูล
...
#### Primary Gateway Board / บอร์ดเกตเวย์หลัก
- **Model**: ESP32-S3-POE-ETH-8DI-8DO (Waveshare)
- **Processor**: ESP32-S3 Dual-Core @ 240MHz (ตัวประมวลผล)
- **Memory**: 16MB Flash + 8MB PSRAM (หน่วยความจำ)
- **Network**: WiFi 2.4GHz, Bluetooth 5.0, Ethernet (PoE) (เครือข่าย)
- **I/O**: 8 Digital Inputs, 8 Digital Outputs (อินพุต/เอาต์พุต)
- **Communication Interfaces**: RS485 (Isolated), CAN (Isolated), USB Type-C (อินเตอร์เฟสสื่อสาร)

#### Remote Data Source / แหล่งข้อมูลระยะไกล
- **Model**: SX1262 LoRa DTU (Rail-mount) (รุ่น)
- **Connectivity**: RS232/RS485/RS422 to LoRa (การเชื่อมต่อ)
- **Frequency Band**: Sub-GHz (ย่านความถี่)
- **Range**: Long-range wireless communication (ช่วงระยะไกล)
- **Link**: https://www.waveshare.com/product/iot-communication/long-range-wireless/nb-iot-lora/sx1262-lora-dtu-lf.htm
...
Remote LoRa Transmitter (ESP32-S3)
         │
         │ [LoRa Protocol]
         │
         ▼
SX1262 LoRa DTU
         ...
         ▼
┌──────────────────────────────────────────┐
│  ESP32-S3-POE-ETH-8DI-8DO Gateway        │
│  (This Project)                           │
├──────────────────────────────────────────┤
│ 1. WiFi Configuration Module              │
...
└──────────────────────────────────────────┘
    ...
    │          │            │             │
    ▼          ▼            ▼             ▼
  MQTT      Google      Telegram       Local
  Broker    Sheets       Bot           Storage
```
...
| Pin | GPIO | Function | Notes / หมายเหตุ |
|-----|------|----------|------|
| BUZZER | GPIO46 | Buzzer Control | PWM-controlled buzzer (PWM 1kHz, 256 levels) |
